...
function initializeGame() {
  createSolutionsGrid();
  drawTable();
  setNumberOfCellsToEmpty(20);
}

function createSolutionsGrid(){
  clearGameGrid();
  var spot;
  setFirstCellInEachRow();
  for (var y = 1; y < CELLS_PER_ROW; y++){
    if (y%3 == 0){
      for (var x = 0; x < 9; x++){
        gameGrid[y][x] = gameGrid[y-1][shiftGridIndicesByNumOfSpaces(x, 3)];
      }
    }
    else{
      for (var x = 0; x < 9; x++){
        gameGrid[y][x] = gameGrid[y-1][shiftGridIndicesByNumOfSpaces(x, 1)];
      }
    }
  }
  var solutionsGrid = JSON.parse(JSON.stringify(gameGrid));
  console.log(solutionsGrid);
}
...
function clearGameGrid() {
  ...
  for (var x = 0; x < 9; x++){
    for (var y = 0; y < 9; y++){
      tempSingleLine.push(EMPTY_SPACE);
    }
    gameGrid.push(tempSingleLine);
    tempSingleLine = [];
  }
}

function shiftGridIndicesByNumOfSpaces(index, spaces) {
  newIndex = index + spaces;
  if (newIndex >= 9) {
    newIndex %= 9;
  }
  return newIndex;
...
function setNumberOfCellsToEmpty(cells){
  var r;
  var c;
  var saveNum;
  var giveUp = 0;
  ...
  for (var x = 0; x < cells; x++){
    ...
    while (test == 0){
      ...
      if (gameGrid[r][c] == 0){
        ...
        saveNum = gameGrid[r][c];
        gameGrid[r][c] = 0;
        if (giveUp > 100){
          test = 1;
        }
        if (isPossible() == false){
          gameGrid[r][c] = saveNum;
  ...
  for (var x = 0; x < 9; x++){
    for (var y = 0; y < 9; y++){
      if (gameGrid[x][y] == 0){
        ...
        else if (Number(value) != solutionsGrid[x][y]){
...
function getAnswer(row, column){
  ...
  var testBox;
  for (var x = 0; x < 9; x++){
    testRow = testInArray(testNum, gameGrid[row][x]);
    if (testRow != -1){
  ...
  for (var x = 0; x < 9; x++){
    testColumn = testInArray(testNum, gameGrid[x][column]);
    if (testColumn != -1){
  ...
  for (var x = testxy * 9; x < (textxy + 1) * 9; x++){
    ...
    testBox = testInArray(testNum, gameGrid[zx][zy]);
    if (testBox != -1){
